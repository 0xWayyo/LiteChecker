...
def platform_smoke(output: Path) -> int:
    ...
    paths = package_platforms.build_sources(output / "archives", version="0.6.3",
        public_key=key, repository="example/LiteChecker")
    snapshots = {}
    for platform, path in paths.items():
        validated = validate_source_zip(path.read_bytes(), expected_platform=platform, expected_version="0.6.3")
    ...
    if platform in {"windows", "macos"}:
        ...
        source = output / f"archives/LiteChecker-0.6.3-{label}.zip"
