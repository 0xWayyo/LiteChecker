"""Fresh subscription measurement with no delivery or acknowledgement state."""
...
async def measure_cycle(
    ...
    try:
        ...
        decision = await state_call(dependencies.snapshot_store.consider, targets, observed_at)
    ...
    sequence = await state_call(dependencies.sequence_store.next)
    duration_ms = max(
        0,
...
def make_measurement_dependencies(
    settings: ProbeSettings, *, state_dir: Path
