...
if not exist "%LITECHECKER_SCRIPT%" (
  echo LiteChecker Windows trial launcher is incomplete. Extract the whole ZIP first.
  exit /b 2
)
...
"%LITECHECKER_POWERSHELL%" -NoLogo -NoProfile -ExecutionPolicy Bypass -File "%LITECHECKER_SCRIPT%" -Root "%LITECHECKER_ROOT%."
set "LITECHECKER_RESULT=%ERRORLEVEL%"
if not "%LITECHECKER_RESULT%"=="0" (
  echo.
  echo LiteChecker Windows trial finished with code %LITECHECKER_RESULT%.
)
pause
