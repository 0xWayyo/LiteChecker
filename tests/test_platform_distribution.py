...
def sources(tmp_path):
    ...
    paths = script("package_platforms").build_sources(
        tmp_path / "sources", version="0.6.3", public_key=key.public_key().public_bytes_raw(),
...
def test_runtime_profile_preserves_pins_hashes_and_bytes_across_checkout_line_endings(tmp_path, monkeypatch, line_ending):
    ...
    expected = builder._runtime_metadata("0.6.3")
    ...
    actual = builder._runtime_metadata("0.6.3")
...
def test_crlf_runtime_lock_still_rejects_missing_allowed_dependency(tmp_path, monkeypatch):
    ...
    with pytest.raises(ValueError, match="runtime lock allowlist is incomplete"):
        builder._runtime_metadata("0.6.3")
...
def test_sign_all_targets_and_windows_wrapper_preserves_payload(sources, tmp_path):
    ...
    for platform, path in paths.items():
        ...
        release.build_release(archive=path, private_key=private, output=output,
            version="0.6.3", sequence="8", repository="example/LiteChecker", platform=platform)
        manifest = json.loads((output / f"release-{platform}.json").read_bytes())
        assert manifest["schema"] == 2
        payload = manifest["payload"]
        assert (payload["platform"], payload["version"], payload["sequence"]) == (platform, "0.6.3", 8)
    ...
    wrapper = tmp_path / "LiteChecker-0.6.3-Windows.zip"
...
def test_profile_builder_never_reads_local_secrets_and_outputs_are_immutable(tmp_path, monkeypatch):
    ...
    options = dict(version="0.6.3", public_key=b"k" * 32, repository="example/LiteChecker")
...
def test_one_command_builds_all_public_artifacts_without_overwriting(tmp_path):
    ...
    args = ["build-platforms", "--version", "0.6.3", "--sequence", "8", "--repository", "example/LiteChecker", "--private-key", str(private), "--output", str(output)]
    assert release.main(args) == 0
    assert {p.name for p in output.glob("*.zip")} == {
        "LiteChecker-0.6.3-Windows.zip", "LiteChecker-0.6.3-macOS.zip", "LiteChecker-0.6.3-Linux.zip", "LiteChecker-0.6.3-windows-update-source.zip", "LiteChecker-0.6.3-macos-update-source.zip",
    }
    before = {p.name: p.read_bytes() for p in output.iterdir()}
    assert len(before) == 13
    mac_manifest = json.loads(before["release-macos.json"])["payload"]["artifact"]
    assert mac_manifest["urls"] == ["https://github.com/example/LiteChecker/releases/download/v0.6.3/LiteChecker-0.6.3-macos-update-source.zip"]
    assert mac_manifest["sha256"] == hashlib.sha256(before["LiteChecker-0.6.3-macos-update-source.zip"]).hexdigest()
...
def test_signing_rejects_profile_mismatch_before_outputs(sources, tmp_path, case):
    ...
    with pytest.raises(ValueError):
        script("release").build_release(archive=paths["macos"], private_key=private, output=output,
            version="0.6.4" if case == "version" else "0.6.3", sequence="8",
...
def mac_source(tmp_path):
    key = Ed25519PrivateKey.generate()
    paths = script("package_platforms").build_sources(tmp_path / "sources", version="0.6.3",
