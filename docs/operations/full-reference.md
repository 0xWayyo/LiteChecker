...
[обновления](updates.md), выпуск релизов — [инструкция владельца](https://github.com/0xWayyo/LiteChecker/blob/main/docs/development/releases.md).
...
Пошаговая инструкция: [установка агента](../installation/agent.md), кратко — [НАЧНИТЕ-ЗДЕСЬ.txt](../../НАЧНИТЕ-ЗДЕСЬ.txt). Распакуйте архив, откройте `INSTALL.command` (macOS), `INSTALL.bat` (Windows/WSL2) или `bash INSTALL.sh` (Linux). В готовом PRIVATE-архиве общие секреты уже заполнены. **macOS по умолчанию устанавливает фоновый native DIRECT, Docker не нужен.** Windows/WSL2 и Linux пока используют Docker и текущий системный маршрут; обход включённого VPN там не реализован.
...
Условия, ограничения и разрешение Gatekeeper: [пробный прямой режим](direct-trial.md).
