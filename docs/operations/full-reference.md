...
Версия 0.6.0 использует отдельные подписанные каналы обновлений для macOS,
...
Версия 0.6.0 устанавливается отдельно от старых тестовых копий. Перед запуском новой остановите старую; автоматического переноса данных и сброса старого канала нет. Установленные версии обновляйте через меню.
...
Откройте `LiteChecker.bat`, пройдите мастер и запустите проверки первым пунктом меню. WSL, Docker и права администратора не нужны. Проверки начинаются сразу после запуска и повторяются каждые 10 минут. Закрытие меню не останавливает мониторинг; после перезагрузки Windows его нужно запустить вручную. Настройки, отчёты и диагностика находятся в `_app/windows-state`.
