...
В версии 0.2.0 добавлен механизм подписанных автообновлений и безопасной
очистки старых управляемых версий. Он включается при наличии настроенного
канала публикации; без него статус — `unconfigured`. Инструкции тестеру —
[UPDATES.md](UPDATES.md), выпуск релизов — [UPDATE-RELEASES.md](UPDATE-RELEASES.md).

LiteChecker проверяет доступность каждого VLESS/REALITY-выхода и SNI-доменов из нескольких сетей. Основной способ запуска — **standalone**: каждый компьютер самостоятельно обновляет подписку и пишет в общий Telegram-чат через один и тот же бот-токен. Устройство получает постоянный уникальный ID; город и имя тестера/сети задаются отдельно, в одном городе может быть много устройств.

## Быстрый запуск и перенос

Пошаговая инструкция: [INSTALL-AGENT.md](INSTALL-AGENT.md), кратко — [НАЧНИТЕ-ЗДЕСЬ.txt](НАЧНИТЕ-ЗДЕСЬ.txt). Распакуйте архив, откройте `INSTALL.command` (macOS), `INSTALL.bat` (Windows/WSL2) или `bash INSTALL.sh` (Linux). В готовом PRIVATE-архиве общие секреты уже заполнены. **macOS по умолчанию устанавливает фоновый native DIRECT, Docker не нужен.** Windows/WSL2 и Linux пока используют Docker и текущий системный маршрут; обход включённого VPN там не реализован.
...
Если Telegram недоступен напрямую, сохраните адрес прокси в `secrets/telegram_proxy_url` с правами `0600`. Формат: `socks5://LOGIN:PASSWORD@IP:PORT` (поддерживаются также `socks5h://`, `http://` и `https://`). Спецсимволы в логине/пароле должны быть URL-encoded. В готовом PRIVATE-архиве этот общий секрет может быть уже заполнен: вводить его на каждом устройстве не нужно.

Native DIRECT и `TRY-DIRECT.command` обнаруживают файл автоматически. В постоянной macOS-установке секрет лежит в `~/Library/Application Support/LiteChecker/secrets/telegram_proxy_url`; существующий файл сохраняется при обновлении. В Docker `bash run.sh start` / `check` подключают отдельный `compose.telegram-proxy.yml`, который монтирует только этот секрет; существующая `.env.standalone` не перезаписывается. После добавления/изменения прокси перезапустите соответствующий режим. WSL-установщик переносит общий прокси, но сохраняет уже настроенный локальный.
...
Без файла прокси сохраняется обычная отправка. Если файл настроен, но некорректен, запуск завершается ошибкой; если сам прокси недоступен, отправка повторяется с ним, **без скрытого перехода напрямую**. Постоянный native DIRECT и обычный standalone сохраняют ограниченную очередь на диске; разовая DIRECT-проба не обещает доставку после своего завершения.
...
PRIVATE-архив с токеном бота, подпиской и паролем прокси передавайте только доверенным тестерам. Публичная сборка исключает эти секреты и локальное состояние устройств.
...
4. Скопируйте `agents.example.json` в `agents.json`, замените демонстрационный токен на сгенерированный и выполните `chmod 600 agents.json`.
...
Для запуска только агента передайте также `.env.agent.example`, `.env.compose.example`, `compose.agent.example.yml`, `Dockerfile`, `.dockerignore`, `pyproject.toml`, `uv.lock` и `src/`. На втором host:
...
## Экспериментальный прямой режим macOS

Для разовой пробы при включённом стороннем VPN: `TRY-DIRECT.command`.
Автоматически готовит отдельный нативный runtime, не меняет VPN или маршруты,
проводит одну проверку и отправляет явно экспериментальный отчёт.
Это **не гарантированный обход любого VPN**. Основной `INSTALL.command` теперь устанавливает постоянный DIRECT; `TRY-DIRECT.command` остаётся отдельной разовой пробой.
Условия, ограничения и разрешение Gatekeeper: [DIRECT-TRIAL.md](DIRECT-TRIAL.md).
